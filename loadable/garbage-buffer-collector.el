...
(defcustom garbage-buffer-collection-delay 3
  "Number of hours to delay a garbage buffer collection."
...
(defconst garbage-buffer-collector-timer
  (let ((timer (timer-create))
        (seconds (* garbage-buffer-collection-delay 60 60)))
...
(defun garbage-buffer-collector-set-delay (hours &optional minutes)
  "Updates a delay to run a garbage buffer collection."
  (cl-assert (natnump hours))
  (let ((timer garbage-buffer-collector-timer)
        (seconds (* (+ (* hours 60) (or minutes 0)) 60)))
    (timer-set-idle-time timer seconds)))
...
(defun garbage-buffer-sort (buffers)
...
(defun garbage-buffer-reduce (buffers)
...
which buffers should be released last."
  ...
  (let* ((threshold garbage-buffer-memory-threshold)
         (buffers (garbage-buffer-sort buffers))
...
reduced by `garbage-buffer-reduce' if NO-REDUCE argument is nil."
  (interactive)
  (when-let* ((buffers (if garbage-buffer-query
    ...
    (when-let* ((reduced (if no-reduce
                             collectible
                           (garbage-buffer-reduce collectible)))
                (mem-to-release (garbage-buffer-mem-size reduced)))
      (mapcar #'kill-buffer reduced)
      (message "%s bytes of memory is realsed after a garbage buffer collection"
               mem-to-release)
